...
/**
 * Offline-first [EpisodeRepository]: every write lands in Room stamped
 ...
 */
public class SyncingEpisodeRepository
    ...
    constructor(
        ...
        override suspend fun upsert(episode: Episode) {
            val now = clock.now()
            val stamped = episode.copy(updatedAt = now, syncStatus = SyncStatus.PENDING)
            local.upsert(stamped)
            val operation =
                SyncOperation(
                    id = generateOpId(),
                    collectionPath = EpisodeFirestoreMapper.COLLECTION_PATH,
                    documentId = stamped.id,
                    type = SyncOperationType.UPSERT,
                    fields = EpisodeFirestoreMapper.toFields(stamped),
                )
            store.append(SyncOperationCodec.toPersisted(operation, enqueuedAt = now))
        }

        override suspend fun getById(id: String): Episode? = local.getById(id)

        override fun observeByDiaryEntry(id: String): Flow<List<Episode>> = local.observeByDiaryEntry(id)
