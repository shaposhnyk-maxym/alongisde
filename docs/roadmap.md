...
### M12.11 — Генералізована надійність фонової роботи: WorkManager/BGTaskScheduler-оркестратор (розширено, у процесі)
...
- [ ] `core:domain`: `BackgroundWorkScheduler`/`BackgroundJobKind` (3 значення)
- [ ] `androidApp`: `BackgroundSyncWorker`, `AndroidWorkManagerScheduler`,
      Koin-біндинг, виклик `ensurePeriodicSweepScheduled()` в `onCreate()`
- [ ] iOS: `NoOpBackgroundWorkScheduler`, Koin-біндинг
- [ ] `gradle/libs.versions.toml` + `androidApp/build.gradle.kts`: WorkManager
      (тільки `androidApp`, не `commonMain` — плейн Kotlin-інтерфейс не
      тягне платформну залежність)
- [ ] `BackgroundSyncWorkerTest` (Robolectric, `TestListenableWorkerBuilder`
...
- [ ] `DiaryCaptureCoordinator` отримує нову залежність `PairingRepository`
...
- [ ] `retryIncompleteEpisodes(list)` лишається публічним — одноразовий
...
- [ ] `capture()` → `scheduleOneOff(EPISODE_RETRY)`, якщо після обробки
      лишився `needsRetry`-епізод
- [ ] Видалити `DiaryTimelineDataSource.pollIncompleteEpisodes` +
      ...
      лишається без змін
- [ ] Тести: `retryAllIncompleteEpisodes` фільтрує власні/чужі/завершені/
...
- [ ] Мануальна перевірка: swipe from recents з неповним епізодом →
...
- [ ] Новий `PlaceRetryCoordinator` (`feature:places` presentation,
...
- [ ] `PlaceRetryDataSource.kt` видаляється цілком; `PlacesListContainer`
      — прибрати виклик `observeAndRetry`
- [ ] `PlaceImportContainer.accept()` (не `runImport()` — той лише
      будує прев'ю) → `scheduleOneOff(PLACE_RETRY)`, якщо `needsRetry()`
- [ ] Без foreground-nudge для places (слабший кейс, ніж diary)
- [ ] Тести: `PlaceRetryCoordinatorTest` (мігрує з видаленого
...
- [ ] Усі 4 `Syncing*Repository` отримують `BackgroundWorkScheduler`,
      викликають `scheduleOneOff(SYNC_QUEUE_FLUSH)` після `store.append(...)`
- [ ] `FirestorePairingTripDataSource`'s 5с поллер — без змін (ні push-,
      ні pull-половина)
- [ ] Тести: кожен `Syncing*RepositoryTest` — новий кейс на `scheduleOneOff`

**Спільна мануальна перевірка**: airplane mode під час дії, що лишає
щось незавершеним → увімкнути мережу → підтвердити ретрай спрацював
лише після відновлення; `adb shell dumpsys jobscheduler` для
підтвердження periodic sweep реально заплановано після першого запуску.
