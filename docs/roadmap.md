...
**Автоімпорт фото (WorkManager) — скасовано (рішення користувача,
2026-07-22), не відкладено.** Консультація під час планування M12.11
(2026-07-21) знайшла нерозв'язаний продуктовий ризик: автоімпорт без
курації мовчки завантажив би повний Gemini+Storage-пайплайн на будь-яке
фото з діапазону дат поїздки (чеки, випадкові кадри, приватне) без
вибору користувача, плюс реальний Android 14 partial-photo-access
підводний камінь — саме ці ризики й стали підставою рішення взагалі не
робити цю фічу. Ручний SAF-пікер (`OpenMultipleDocuments`,
`PhotoPickerLauncher`) лишається єдиним постійним шляхом захоплення.
...
з M12.5) винесено з цього мілстоуна.** Консультація виявила реальний
нерозв'язаний продуктовий ризик: автоімпорт без курації мовчки
завантажив би повний Gemini+Storage-пайплайн і показав партнеру
БУДЬ-ЯКЕ фото з діапазону дат поїздки (чеки, випадкові кадри,
приватне) — без вибору користувача, плюс реальний Android 14
...
"Allow all"). Спершу відкладено як окремий майбутній мілстоун (номер
TBD); користувач остаточно вирішив (2026-07-22) фічу взагалі не
робити — саме ці ризики й стали підставою відмови. Наявний ручний
SAF-пікер (`OpenMultipleDocuments`) лишається єдиним і постійним
шляхом захоплення фото, вже задокументований в M12.5 як реальна
фіча, не заглушка до автоімпорту.
...
**Автоімпорт фото — скасовано (рішення користувача, 2026-07-22), не
просто відкладено.** Чернетка Accept-критеріїв з M12.5/M12.11 (курація/
приватність/Android 14 partial-access ризики, які й були підставою для
відкладення) видалена звідси — ручний SAF-пікер (`OpenMultipleDocuments`,
`PhotoPickerLauncher`, див. M12.5) лишається єдиним і постійним шляхом
захоплення фото, не тимчасовою заглушкою до автоімпорту.
...
**Частина 0 (нова, не в оригінальному плані): PlaceSwipe sync-редизайн
✅ done.** Дослідження перед реалізацією виявило, що
`PlaceCandidate.ownerSwipe`/`memberSwipe` (лишились з M1/M13) — два
мутабельних поля ОДНОГО спільного документа, який M13.2's
`SyncingPlaceCandidateRepository` синкає цілим документом (Room
`REPLACE` → повний Firestore push). Якщо owner і member обидва
свайпають офлайн майже одночасно, пізніший push міг би мовчки затерти
ще-не-синканий свайп іншої сторони — той самий клас "whole-document
overwrite" ризику, що вже ловили в M12.11 (`getActiveTrip`-гонка), тут
на рівні полів документа. Рішення користувача: не залишати це як
TODO, а переробити зараз, до появи реальних даних у Matcher.

Замінено на окрему сутність `PlaceSwipe` — один запис на пару
(candidateId, userId), `id = "$candidateId::$userId"` (детермінований,
той самий ідіом, що `DiaryCaptureCoordinator.deterministicNewEntryId`).
Кожен користувач пише ЛИШЕ свій власний запис — структурна гарантія
відсутності конфлікту замість дисципліни коду. `PlaceCandidate` втратив
`ownerSwipe`/`memberSwipe` повністю (Room-міграція v14→v15, той самий
"rename → create → copy → drop" підхід, що `MIGRATION_8_9`; реальних
даних не втрачено — Matcher ніколи не був у продакшені). Новий стек
(`PlaceSwipeRepository`/`SyncingPlaceSwipeRepository`/
`PlaceSwipeFirestoreMapper`/`PlaceSwipeSyncEntityBinding`) дзеркалить
`PlaceCandidate`'s один-в-один. Нова Firestore rules-секція
`placeSwipes` — критична частина дизайну: `request.resource.data.userId
== request.auth.uid` — без цього "пиши лише свій userId" було б
клієнтською конвенцією, не security boundary.

**Accept — Частина 1 (залежить від Частини 0) ✅ done:**
- [x] Юніт-тести правил переходу картки: обоє "так" на ту саму картку →
  переходить у Match-лист; обоє "ні" → картка прибирається з колоди
  назавжди; розійшлись (один так, один ні) → картка лишається в
  колоді й показується знову пізніше. `resolveMatchStatus` (лишився з
  M1/M13) уже покривав саму класифікацію — новим тут є те, що спліт
  (LIKE+DISLIKE) і "ще ніхто не свайпнув" навмисно потрапляють у ту
  саму категорію `PENDING`/колода, тож жоден `PlaceSwipe`-запис не
  потребує скидання чи повторного запису іншою стороною
- [x] `feature:matcher`: `MatcherState`/`MatcherIntent`/`MatcherSideEffect`/
  `MatcherContainer` (за зразком `PairingContainer`) — `deck`/`matches`
  похідні, ніколи не збережені (той самий патерн, що `PairingState.step`).
  `MatcherSideEffect.Matched` фаєриться на щойно-новий match, незалежно
  від того, чи причина — власний свайп, чи партнерів, що прилетів через
  синк
- [x] Orbit-контейнер тестується через `orbit-test`: послідовність
  Intent → State для повного циклу (N карток, включно з кейсом
  "розійшлись → показ повторно → нарешті збіглись")
- [x] Межові випадки: порожня колода (усі картки або matched, або
  removed), одночасний свайп обома на останню картку в колоді —
  тепер тривіально безпечний за конструкцією (кожен свайп — запис у
  свій власний PlaceSwipe-документ), тест підтверджує коректний
  фінальний стан незалежно від порядку прибуття обох свайпів
- [x] `MatcherModule.kt` (Koin DI), `feature:matcher` підключено до
  `androidApp` — без UI (M15), але контейнер повністю резолвиться в
  DI-графі вже зараз

---

### M15 — Matcher: UI (swipe + Match-лист) ✅ done
`feature:matcher` (частина 2) — екран свайпу і список взаємних
матчів. Дизайн — за `design/main_app_design/Alongside - Main App and
Recap (standalone).html` (Screen 17/18): картка в стилі "paper" з
двома декоративними стос-картками позаду, кругові кнопки
`CircleIconButton` (вже існували в `core:ui`, побудовані заздалегідь
саме під цей екран), Match-лист — `PaperCard` + `MediaListRow` + heart-
бейдж.

**Виявлено під час реалізації**: `MatcherState.deck` сам по собі не
годився як джерело порядку показу — картка, яку я вже свайпнув, а
партнер ще ні, лишається в `deck` (`PENDING`), і наївний рендер
`deck.first()` показав би її знову одразу після мого ж свайпу. Додано
`core:domain`'s `isMyTurn(mine, theirs)` + `MatcherState.myTurnDeck` —
"мені є що вирішувати тут" (свіжі картки, партнер-вже-відповів, і
спліти для перегляду; виключає лише "я вже відповів, чекаю на
партнера"). Порядок показу — окрема локальна черга на UI-рівні
(`MatcherContent`), узгоджена з `myTurnDeck` через `LaunchedEffect`:
свайпнута картка прибирається з фронту одразу, і якщо вона
повертається (спліт), то **в кінець черги**, не на початок — так, як
попросив користувач.

**Accept:**
- [x] Screenshot-тести: картка в колоді, порожня колода, список
  Match-лист (порожній і заповнений стани)
- [x] Compose UI-тест: свайп коректно емітить `Intent.Swipe` з правильним
...
через Google Maps share-лінк (M13.1/M13.2, вже реалізовано) достатньо
зручний як єдиний спосіб додавання. `PlaceCandidate`, створений
імпортом, і так буде видимий Matcher'у (M14) без окремого "додати до
пулу" кроку — Matcher читає всі несвайпнуті кандидати поїздки незалежно
від способу додавання, тож скасування цього мілстоуна нічого не
блокує в M14/M15.
...
### Pre-trip photo — розбито на 4 менші мілстоуни (M19.5-M19.8)

Новий тип сутності, що не прив'язана до `Episode`/`DiaryEntry` (немає
дня поїздки, до якого його прикріпити), тож потребує власного шляху
крізь усі існуючі шари синку, не лише нової Room-таблиці. Замість
одного великого мілстоуна — 4 послідовні (кожен реально залежить від
попереднього, паралелити тут нема сенсу: локальна модель → push →
pull → UI зверху):

1. **M19.5** — модель + Room + repository + upload-клієнт (чисто
   локально, без Firebase)
2. **M19.6** — push-синк (`firestore.rules` + `SyncEntityBinding`)
3. **M19.7** — pull-синк + повний round-trip
4. **M19.8** — Countdown-картка + upload UI (`feature:diary`)

---

### M19.5 — Pre-trip photo: модель, Room, repository, upload-клієнт ✅ done
`core:model`, `core:database`, `core:network`. Чисто локальний і
Storage-upload шар, без жодної Firestore-залежності — мержиться й
працює офлайн самостійно, синк додають M19.6/M19.7.

**Accept:**
- [x] Юніт-тест Room-міграції: нова таблиця `pre_trip_photos` (`tripId`,
      `userId`, `uri`, `takenAt`, `latitude`, `longitude`, `remoteUrl?`,
      `syncStatus`) — round-trip запису
- [x] Юніт-тести repository (insert/query по `tripId`+`userId`,
      syncStatus-переходи) — той самий offline-first патерн, що вже є для
      `PlacePhoto`
- [x] Юніт-тести upload-клієнта проти Ktor `MockEngine` (успіх, 4xx, 5xx,
      timeout) — за зразком `FirebaseStorageUploadClient`
- [x] `storage.rules` — без змін, задокументовано чому (реюз флет-шляху
      `photos/{photoId}`, вже "будь-який автентифікований користувач")

Невеликі, очікувані уточнення понад буквальний список полів: `id`
(Room-таблиці потрібен `@PrimaryKey`, як і в кожній іншій сутності
цього шару) та індекс по парі (`tripId`, `userId`) замість лише
`tripId` — саме так формується основний запит (`observeByTripAndUser`).
`PreTripPhotoUploadClient`/`FirebasePreTripPhotoUploadClient` (аплоад-
клієнт з M19.5.3) реюзає `FirebaseStorageApi`/`FirebaseStorageConfig`
без змін, з префіксом `pretrip_` в object id — той самий підхід, що й
у `FirebasePlacePhotoUploadClient` з префіксом `place_`. DI-реєстрація
(Koin) свідомо не додана — вона зʼявиться разом із sync-обвʼязкою в
M19.6/M19.7, коли зʼявиться реальний споживач.
...
за абстракціями:
1. `firestore.rules` — це hard allowlist із
   `match /{document=**} { allow read, write: if false; }` в кінці. Нова
   колекція без власного `match`-блоку — 403 на кожен read/write, без
   зрозумілого логу (та сама пастка, що вже задокументована в rules-файлі
   для `resource == null`). Блок обов'язковий, за аналогією з
   `diaryEntries`: read — `isTripMember` (з `resource == null`-винятком
   для preflight-GET нового документа); create — `userId == auth.uid` +
   `isTripMember(tripId)`; update/delete — тільки власний `userId`.
2. Реєстрація в `data/di/DataModule.kt` має власну відому пастку:
   `single<SyncEntityBinding> { ... }` (замість `bind SyncEntityBinding::class`)
   мовчки лишає в Koin-реєстрі тільки останній зареєстрований біндинг —
   це вже одного разу гасило Trip/DiaryEntry-синк (див. коментар у
   файлі). Новий `PreTripPhotoSyncEntityBinding` мусить іти через
...
- [x] Новий блок `match /preTripPhotos/{id}` у `firestore.rules` (див. п.1
      ...
      фікс. **Виконано користувачем**: rules задеплоєні; крос-акаунт
      перевірка — через Firebase Console Rules Playground (UI для
      створення pre-trip photo ще нема, з'явиться в M19.8, тож реального
      app-based смоуку поки не існує): `create` як власник документа
      (`userId` == uid) в межах спільного `tripId` — allow; `get` тим же
      документом іншим учасником поїздки — allow; `update` цим самим
      документом іншим учасником (не власний `userId`) — deny; `get`
      користувачем поза поїздкою — deny. Усі чотири сценарії підтверджені
...
Pull-синк — НЕ генеричний. `DiaryContentPuller`/`PlaceContentPuller`
...
- [x] Новий `PreTripPhotoContentPuller` (`core:domain`) + Firestore-реалізація
  ...
  M15.5): партнерський запис підтягується; власний, якого немає
  локально, підтягується; власний, що вже є локально, ніколи не
...
- [x] **Повний push→pull round-trip, не лише кожна половина ізольовано**:
  ...
  `remoteUrl`, не `content://...` (той самий фікс-шлях, що в M12.5), і
...
M20-слайду "паралельні життя" — потрібен масив фото "до зустрічі", які
не прив'язані до конкретного дня поїздки (на відміну від `Episode`, який
завжди належить `DiaryEntry` в межах `trip.startDate..endDate`).

`Countdown`-картка (`DiaryTimelineState.kt`) вже існує й показується
рівно поки `daysUntilReunion > 0`, зникаючи назавжди щойно доходить до
0 — природне місце для CTA "додати фото". На відміну від епізодів,
кластеризація не потрібна: кожне фото — окремий запис зі своїм
`takenAt`/lat-lon, без `episodeId`.

**Перевірити окремо: картка ховається назавжди після старту поїздки,
користувач більше в принципі не має її бачити — не тільки "немає в
списку", а взагалі ніде.** Існуючий інваріант (`items` — `Derived,
never stored`, `daysUntilReunion > 0`) сам по собі вже це гарантує для
голого Countdown-айтема, але тепер картка носить реальний стан (фото,
дві кнопки, upload-flow), тож варто явно пере-перевірити, що більший
обсяг стану нічого не тримає штучно живим:
- Наявність pre-trip фото (0 чи N) не впливає на умову зникнення —
  повний, заповнений фотками Countdown-айтем зникає рівно так само, як
  порожній, щойно `daysUntilReunion` доходить до 0
- Немає окремого шляху назад до picker'а цієї картки після зникнення —
  ні через збережений nav-стан, ні через діалог, що міг лишитись
  відкритим
- Edge case "дата перевалила рівно під час відкритого picker'а" не
  потребує окремої обробки: `captureDate`/еквівалент фіксується в
  момент тапу (той самий патерн, що вже є для `ProcessCapturedPhotos`
  у `DiaryTimelineScreen.kt` — `captureDate` захоплюється до виклику
  пікера, не читається заново після), тож дозволений in-flight upload
  завершується нормально, а не губиться й не "оживляє" зниклу картку

**Кнопки — та сама пара, що на day-картках, навіть якщо семантика
"закрити" тут не буква в букву день.** `CaptureButtonArea`
(`DiaryTimelineScreen.kt`) на вибраній day-картці зараз показує два
кнопки — "Додати фото" (`onAddPhotosClick`) і "Закрити день"
(`onCloseDay`). Поки бекграунд-пайплайн і background-sync ще на етапі
дебагу (мануальні тригери — той самий дух, що M12.6's Close Day й
M12.11's WorkManager-надійність), Countdown-картка теж отримує обидві
кнопки в тому ж вигляді, хоч це й не "день" у сенсі `DiaryDayCard` —
потрібно мати ручний спосіб форсувати обробку/синк без очікування
автоматики. Що саме робить "Закрити" для pre-trip-бакета (чи є там
взагалі еквівалент "закритого" стану) — відкрите питання на момент
...
### Recap — розбито на 4 менші мілстоуни (M20-M20.3)

Формування рікапу — не робота фічі, там і так вистачає власних проблем
(Stories-хром, тап-навігація, auto-advance). Розподіл — за тим самим
принципом, що вже діє для Diary (`buildDiaryTimelineDays`/
`resolveDayUnlockState` в `core:domain`, а `feature:diary` лише мапить
результат у власний presentation-стан):

1. **M20** — слайд-модель і генерація (`core:model` + `core:domain`)
2. **M20.1** — доступність і тригер (`Recap`-сутність, Home-гейт)
3. **M20.2** — Stories-хром (`core:ui` + `playground`)
4. **M20.3** — інтеграція (`feature:recap`)

**M20, M20.1, M20.2 не залежать одне від одного — можна робити
паралельно/в будь-якому порядку.** Жоден з них не потребує коду з
інших: слайд-генерація (M20) — чисті функції над уже наявними
репозиторіями (`Episode`, `PlaceSwipe`, `PreTripPhoto`, `Trip`), без
жодного знання про `Recap`-сутність чи UI; доступність (M20.1) —
окрема сутність і гейт, що не знає про `RecapSlide` взагалі; хром
(M20.2) — generic UI над плейсхолдерами, не знає ні про перше, ні про
друге. **M20.3 залежить від усіх трьох** — це той мілстоун, де
`feature:recap` бере готову деку з M20, гейт з M20.1 і хром з M20.2 та
збирає їх в один реальний екран.

**Наскрізна залежність від Pre-trip photo (M19.5-M19.8):** `M20`'s
слайд "паралельні життя" — чиста функція, приймає списки `PreTripPhoto`
як аргументи, тож сам по собі тестується й на фейкових списках, без
жодної залежності від M19.x. Реальна залежність — у `M20.3`: там, щоб
`RecapContainer` реально скомпілювався й опитав `PreTripPhoto`-репозиторій,
потрібен щонайменше `M19.7` (дата-шар з pull-синком); а щоб цей слайд
колись реально мав що показати (не завжди деградував до відсутнього),
потрібен ще й `M19.8` (UI, яким хтось насправді додасть фото).
...
- **`core:model`** — сутність `RecapSlide` — sealed-ієрархія, по
  одному data-класу на кожен тип екрана/слайду (`Intro`,
  `ParallelLives`, `DayHighlight`, `ClosestMoment`, `SwipeArchetype`,
  `UnresolvedQuestion`, `MatchList`, `Final`) з усіма полями,
  потрібними для рендеру. Ніяких UI-полів тут — тільки дані
- **`core:domain`** — `buildRecapSlides(...)` і всі під-функції нижче
  (правила мінімуму на слайд, haversine-util, вибір pre-trip пари з
  макс. відстанню, свайп-архетип, детекція незакритого, підрахунок
  днів разом). Усе тестується в `commonTest`, без жодного
  Compose/Container довкола

**Мінімальний рікап — деградація при відсутніх даних.** Тригер —
дата останнього дня маршруту, не факт наявності контенту. День може
розблокуватись і без жодного `Episode` (`MISSED`-стан, M12.12: дата
минула — день "закритий" самим лапсом часу, не `Close Day`-тапом), а
`Countdown`-картку (M19.8) міг не заповнити ні один, ні другий
учасник. Тобто кожен тип слайду — чиста функція від наявних даних:
немає з чого зібрати конкретний слайд → він просто відсутній у
послідовності, без плейсхолдера "немає даних" і без падіння генерації.
Мінімальний випадок (нуль епізодів, нуль pre-trip фото, нуль свайпів з
обох сторін — трип, яким по суті не користувались) — це **Intro +
Final і більше нічого**, обидва обчислювані винятково з `Trip.startDate/endDate`,
без залежності від згенерованого користувачами контенту.

Правило мінімуму на кожен тип слайду:
- **Intro** — завжди є (список міст-точок може бути порожнім, слайд
  однаково рендериться, без міст)
- **Паралельні життя** — потрібен ≥1 `PreTripPhoto` **з обох сторін**;
  бракує хоч одній — слайду немає (немає пари для порівняння "хто де
  був", половинчастий варіант сенсу не має)
- **Day highlight** — потрібен ≥1 `Episode` (з будь-якої сторони) саме
  на цю дату; дата без жодного епізоду з обох боків — без слайду,
  решта дат не зачіпає
- **Найближчий момент** — потрібна хоч одна дата з перетином
  `startTime..endTime` епізодів обох користувачів за весь трип; якщо
  такої дати немає жодної — слайду немає
- **Свайп-архетип** — потрібен ≥1 категоризований `PlaceSwipe` **з
  обох сторін** (порівняння двох профілів смаку, не одного)
- **Незакрите питання** — потрібен ≥1 `PlaceCandidate` зі статусом
  PENDING/одностороннім на момент генерації; відсутність — це очікуваний
  "добрий" випадок (усе зійшлось), не помилка
- **Match-лист** — потрібен ≥1 `MATCHED` `PlaceCandidate`; нуль матчів
  — слайду немає, не порожня сітка
- **Фінал "X днів разом"** — завжди є, ніколи не пропускається

Припущення (легко змінити, якщо не так): нумерація "Day N" на
day-слайдах лишається календарним `dayIndex`, не перенумеровується
послідовно серед показаних — стрибок "Day 1" → "Day 4" вважається
чеснішим за фейкову суцільність, а не багом.

**Accept — усе нижче є жорсткою вимогою, не побажанням: без відповідного
теста пункт вважається невиконаним.**
- Юніт-тест `buildRecapSlides(...)`: повна послідовність із заповнених
  даних поїздки (правильна кількість day-слайдів, взаємно підтверджені
  місця з Match-листа присутні, фінальний слайд завжди останній)
- Спільний haversine-util — юніт-тест на відомих парах координат
  (включно з граничним випадком: та сама точка → 0)
- Юніт-тест слайду "паралельні життя": з пар `PreTripPhoto` (моє ×
  партнерове, найближчі за часом) вибирається пара з максимальною
  haversine-відстанню
- Юніт-тест слайду "найближчий момент": для кожної дати поїздки —
  перетин `startTime..endTime` епізодів обох користувачів →
  мінімальна haversine-відстань за весь трип
- Юніт-тест слайду "свайп-архетип": розподіл `PlaceSwipe.direction` по
  `PlaceCandidate.category`, окремо для owner і member
- Юніт-тест слайду "незакрите питання": кандидати, чий статус на
  момент генерації рекапу лишився PENDING або одностороннім
- Юніт-тест фінального слайду "X днів разом":
  `trip.endDate - trip.startDate + 1`
- **Юніт-тест мінімального рікапу**: нуль епізодів, нуль pre-trip
  фото, нуль свайпів з обох сторін → послідовність = рівно `[Intro, Final]`
- Юніт-тест на кожен тип слайду окремо (не один загальний, кожен
  своїм тестом): відсутність його мінімуму даних (див. таблицю вище)
  прибирає лише цей слайд, решта послідовності не ламається й не
  скорочується
- Юніт-тест: `PreTripPhoto` є тільки в одного з двох учасників →
  "паралельні життя" відсутній (не однобічний варіант)
- Юніт-тест: категоризовані свайпи є тільки в одного з двох учасників
...
учасники рахують той самий дедлайн з тих самих (уже спільних)
`Trip.startDate/endDate`, жодного нового Firestore-шляху не треба — на
відміну від `PreTripPhoto` (M19.5-M19.7), тут немає користувацького
контенту, яке треба донести до партнера, тільки похідне число, яке
кожен пристрій і так порахує однаково.

- `Recap(tripId, availableAt: LocalDate)` (`core:model`) — один рядок
  на трип, `core:database`-Room зберігає, без Firestore-мапера, без
  `SyncEntityBinding`, без пуллера
- `recapAvailableAt(tripEndDate) = tripEndDate + 1 day` (`core:domain`,
  чиста функція) — від уже синканого `Trip`, тому й не потребує синку
  сама по собі
- Формується в момент, коли розблоковується останній день маршруту
  (`date == trip.endDate`, обидві сторони закрили): той самий крок, що
  зараз пише unlock-стан цього дня, атомарно вставляє (upsert, не
  падає, якщо вже є) рядок `Recap`. Це не новий `DiaryDayWaitingState`
  і не асинхронне очікування — чистий локальний Room-запис без мережевої
  залежності, тож на практиці миттєвий
- **Останній день не показується, поки цей `Recap`-рядок не
  записаний** — навмисно зв'язано в одну атомарну дію, щоб не було
  стану "останній день видно, а що далі — незрозуміло"
- Home (`AlongsideApp.kt`, зараз `PlaceholderScreen` з кнопкою
  "Recap", що безумовно робить `backStack.add(Recap)`) отримує тільки
  одну малу зміну: кнопка/вхід на реальний рікап показується лише коли
  `today >= recap.availableAt` для активного трипу; до цього — без
  входу (чи явно задизейблений). Решта Home лишається плейсхолдером,
  як є — це не привід переробляти весь екран
- Push-нотифікація "ваш рекап готовий" (concept.md) — свідомо поза
  скоупом цього мілстоуна; для MVP доступність рікапу — це просто факт
  появи входу на Home після `availableAt`, без окремого сповіщення

**Accept:**
- Юніт-тест: `recapAvailableAt(tripEndDate) == tripEndDate + 1 day`
- Юніт-тест "чи треба планувати рікап зараз" (`core:domain`, чиста
  функція): так лише коли `date == trip.endDate` і обидві сторони
  закрили; ні — для будь-якого іншого дня чи неповного закриття
- Юніт-тест Room: `Recap(tripId, availableAt)` round-trip
- Юніт-тест: upsert ідемпотентний — повторний виклик на вже існуючий
  рядок не падає, не дублює, не міняє `availableAt`
- Юніт-тест `feature:diary`-тригера (мірорить
  `PlaceContentPullCoordinatorTest`'s патерн "викликається/не
  викликається"): розблокування дня з `date == trip.endDate` реально
  викликає `RecapRepository.ensureScheduled(tripId, availableAt)` рівно
  один раз; будь-який інший день чи неповне закриття — не викликає
  жодного разу
- Юніт-тест: день `date == trip.endDate` лишається не-`UNLOCKED`, поки
  `ensureScheduled` не повернувся успішно; після — переходить в
  `UNLOCKED`
- Юніт/Compose-тест Home-гейта (`app`): `today < recap.availableAt` →
  вхід на реальний рікап не показується; `today >= recap.availableAt`
  → показується; немає `Recap`-рядка взагалі → так само не
  показується (не крашиться на відсутньому рядку)
...
`core:ui` отримує сам Stories-хром: прогрес-бари зверху, тап-зони
зліва/справа, auto-advance-таймер, переходи (scale+fade, паралакс
фото — concept.md item 10). Це компонент над "список слайдів ⇒
композабл на слайд", жодного знання про `RecapSlide`.

**Порядок розробки — спочатку `playground`, за тим самим прецедентом,
що M12.9's `Zoomable`/`FullscreenPhotoViewer`.** Хром збирається й
ітерується в `playground` першим — кольорові плейсхолдери замість
реальних фото й даних, живий Hot Reload на самій механіці
жестів/таймінгу/переходів, без жодної залежності від `core:domain`/
`feature:recap`. Тільки після того, як хром у `core:ui` живе й
перевірений там мануально, `M20.3` підключає до нього реальні дані.

**Accept:**
- Compose UI-тест тап-навігації (тап зліва/справа рухає індекс) і
  auto-advance таймера (з можливістю прискорити тест — не чекати
  реальні 5-7 секунд)
- Screenshot-тест самого хрому (прогрес-бари, стан "N з M") на
  плейсхолдерному контенті
...
### M20.3 — Recap: інтеграція (`feature:recap`) ✅ done
...
щоразу читає поточні локальні дані й будує деку слайдів наживо —
нічого заздалегідь не кешується, тож якщо після формування `Recap`-рядка
(`M20.1`) довантажиться ще щось від партнера (пізній синк), рікап при
відкритті все одно відобразить найсвіжіший стан, не застиглий знімок
моменту закриття. Home-кнопка "Recap" (гейт із `M20.1`) тепер веде на
цей реальний екран замість плейсхолдера.

**Accept:**
- Юніт-тест `RecapContainer`: викликає `buildRecapSlides(...)` з
  реальних даних активного трипу (через `core:domain`-репозиторії, не
  фейкові списки в самому Container-тесті) і мапить результат у свій
...
**Відхилення від опису вище:**
- Дека будується **одноразово** (`.first()` на кожному repository-flow),
  не через довгоживучий `collect`, попри формулювання "щоразу читає
  поточні локальні дані" — це про момент **відкриття** екрана, а не про
  безперервне перебудовування деки поки користувач її дивиться (інакше
  слайди могли б зсунутись під час самого перегляду, якщо саме тоді
  прилетів синк від партнера). Той самий контраст, що вже в `M20.3`'s
  власному описі: `DiaryTimelineContainer` живо стежить за екраном, що
  активно редагується, `RecapContainer` — ні
- `RecapSlideContent` — окрема `internal fun` у `RecapScreen.kt`
  (не інлайнена в сам екран) саме для того, щоб мапинг-тест міг
  рендерити її напряму без повного `RecapContainer`/DI; сам тест
  (`RecapSlideContentTest.kt`) перевіряє вичерпність через унікальний
  статичний заголовок кожного з 8 композаблів M20.3.5 (`"Your trip,\ntogether"`,
  `"Parallel lives"`, `"Day N"`, `"Closest moment"`, `"Your taste"`,
  `"Still deciding"`, `"Your matches"`, `"Days together."`) — реально
  виконувана перевірка, не лише компіляторна гарантія
- `ContainerHost<RecapState, Nothing>` — без окремого `RecapSideEffect`:
  "користувач закрив рікап" (тап на останній слайд / auto-advance до
  кінця) — це чиста навігаційна дія, передана як звичайний callback
  `onFinish` у `RecapScreen`, той самий підхід, що `entry<Settings>`'s
  `onClose`, а не Orbit side effect (немає жодної бізнес-логіки, яку
  туди варто було б класти)
- `RecapModuleMarker.kt` (порожній плейсхолдер-файл модуля) видалений —
  модуль тепер має реальний контент
- **Мануальна E2E-перевірка Home → Recap на емуляторі лишилась
  невиконаною** — той самий чесний клас обмеження, що вже фіксувався
  для попередніх мілстоунів (M5/M7/M13/M17): збірка встановлена й
  запущена на реальному Android-емуляторі (`Pixel_8_Pro.avd`), дійшла до
  екрана Google Sign-In з реальним акаунтом користувача, але мережа
  емулятора в цій сесії виявилась нестабільною (пошкоджені ping-пакети,
  розсинхронізація часу), і довести реальний sign-in → active trip →
  обидва закрили останній день → `Recap`-екран до кінця не вдалося.
  `./gradlew ktlintCheck detekt allTests` (без iOS-таргетів, з тієї ж
  причини, що вже зафіксована в пам'яті сесії — на цій машині немає
  встановленого iOS Simulator runtime) + `:androidApp:assembleDebug` —
  усе зелене; застосунок вручну не пройдений повним флоу Home → Recap.
