...
**Accept — Автоімпорт фото (WorkManager):**
- [ ] Спайк-перевірка задокументована як пройдена: AAR-компоненти
      маніфесту WorkManager коректно мерджаться під
      `com.android.kotlin.multiplatform.library` (AGP 9) цього проєкту
- [ ] Юніт-тести `DiaryImportScannerTest`: немає активної поїздки →
      no-op; фільтрація за діапазоном дат поїздки; дедуп URI проти вже
      персистентних епізодів; групування нових фото по днях з
      коректним числом/аргументами викликів `capture()`; повторне
      використання наявного `DiaryEntry.id` для дня, що вже має запис
- [ ] Юніт-тести дозволів онбордингу: об'єднаний запит
      Photos+`ACCESS_MEDIA_LOCATION` покриває всі 4 статуси дозволу
      (GRANTED/NOT_DETERMINED/DENIED/DENIED_PERMANENTLY)
- [ ] Screenshot-тест: оновлений `@Preview` кроку Photos з об'єднаним
      дозволом → Roborazzi-голден
- [ ] **Чесне обмеження**: `DiaryImportWorker.doWork()` (сама
      WorkManager-обв'язка) не юніт-теститься напряму (той самий клас
      проблеми, що Credential Manager в M5, `ContentResolver` в M10) —
      покрита тільки `DiaryImportScanner` (усе, що після виклику
      Worker'а)
- [ ] Мануальний чекліст: періодичний імпорт реально відпрацьовує на
      пристрої/емуляторі, фото з'являються без ручного пікера
...
### M12.11 — WorkManager-backed capture retry (заплановано, не
    реалізовано)

Перенести `retryIncompleteEpisodes` з in-memory поллінгу
(`DiaryTimelineDataSource`, живе лише поки живий Container) на
`WorkManager`-задачу (Android; на iOS — `BGTaskScheduler` через
спільний `expect`/`actual` seam, як і решта платформо-специфічних
речей у цьому проєкті) — переживає закриття застосунку, вбивство
процесу, перезавантаження пристрою.

**Accept (чернетка, уточнити перед реалізацією):**
- `WorkManager`-задача (`CoroutineWorker`), enqueue'иться після кожного
  `capture()` з незавершеним епізодом (а не лише по таймеру) +
  періодична backup-задача (`PeriodicWorkRequest`) на випадок, якщо
  enqueue сам не спрацював
- Constraint на `NetworkType.CONNECTED` — не намагатись, поки мережі
  явно нема, замість поллінгу наосліп
  кожні 30с
- Використовує ту саму `DiaryCaptureCoordinator.retryIncompleteEpisodes`
  логіку (без дублювання) — лише транспорт з in-memory поллінгу на
  `WorkManager` міняється
- Тест: `CoroutineWorker` викликає `retryIncompleteEpisodes` з
  правильним списком епізодів; `Result.retry()` при `WorkerFailure`,
  `Result.success()` коли нема що виправляти чи все виправлено
- Мануальна перевірка: закрити застосунок (swipe from recents) з
  неповним епізодом → почекати → відкрити знову → епізод вже
  довантажений/має опис без додаткової дії користувача
